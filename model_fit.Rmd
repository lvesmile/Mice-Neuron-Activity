...
zm_drop$V2 <- NULL
df<-sort(sample(1:nrow(zm_drop), nrow(zm_drop)*.))
...
open_data <- data.frame(open = train_open[,"V1"],pca_zm$x[,1:23])
names(train_open2)[1]<-'open'
glm_fit = glm(open~., data=train_open2, family = binomial("logit") )
summary(glm_fit)
par(mfrow = c(2,2))
plot(glm_fit)
```
